...
class ProfileUpdateMutation(graphene.Mutation):
    ...
    def mutate(self, info, profile_update_input):
        try:
            ...
            previous_profile.image = profile_input.image if profile_input.image != None else previous_profile.image
            ...
            return ProfileUpdateMutation(profile=previous_profile, success=True, error=None)

        except CustomUser.DoesNotExist:
            ProfileUpdateMutation(profile=None, success=False,
...
class ProfileQuery(graphene.ObjectType):
    profiles = graphene.Field(ProfileType)  # returns profile
...
class Query(UserQuery, ProfileQuery, MeQuery, LogoutQuery, QueryUser, graphene.ObjectType):
